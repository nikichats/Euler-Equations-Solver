...
def MacCormack(density, momentum, energy, ghost_nx, nx, gamma, dt, dx):
    ...
    # (1) prediction step
    [flux1_density, flux1_momentum, flux1_energy] = flux_fc(density, momentum, energy, gamma)
    for i in range(ghost_nx, nx - ghost_nx):
...
def RichtMyer_Smart(density, momentum, energy, ghost_nx, nx, gamma, dt, dx, CFL):
    density_new = np.zeros(nx)
    momentum_new = np.zeros(nx)
    energy_new = np.zeros(nx)
    density_predictor = np.zeros(nx)
    ...
    for i in range(ghost_nx, nx - ghost_nx):
        density_predictor[i] = 0.5 * (density[i + 1] + density[i]) -   (0.5 * dt/dx) * (flux1_density[i+1] - flux1_density[i])
        momentum_predictor[i] = 0.5 * (momentum[i + 1] + momentum[i]) -   (0.5 * dt/dx) * (flux1_momentum[i+1] - flux1_momentum[i])
        energy_predictor[i] = 0.5 * (energy[i + 1] + energy[i]) -   (0.5 * dt/dx) * (flux1_energy[i+1] - flux1_energy[i])
